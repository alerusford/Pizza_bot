...
async def make_changes_command(message: types.Message):
    await bot.send_message(admin_id, 'Что надо?', reply_markup=admin_kb.button_case_admin)
...
async def cm_start(message: types.Message):
    if message.from_user.id == int(admin_id):
        ...
        await message.reply(f'у {message.from_user.id} нет доступа!')
...
# ловим первый ответ и добавляем в словарь
...
async def delete_item(message: types.Message):
    if message.from_user.id == int(admin_id):
        ...
        for ret in read:
            await bot.send_photo(message.from_user.id, ret[0], f'{ret[1]}\nОписание: {ret[2]}\nЦена: {ret[-1]}', reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton(f'Удалить {ret[1]}', callback_data=f'del {ret[1]}')))
